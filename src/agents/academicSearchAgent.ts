...
const createBasicAcademicSearchAnsweringChain = (
  ...
  const rerankDocs = async ({
    ...
    // NOTE (section 1.3 audit): this sort is ASCENDING (a - b), so after the
    // >0.5 filter, .slice(0, 15) keeps the LEAST similar of the qualifying
    // docs rather than the most similar. This is the reference implementation
    // as given — left unchanged here per the assignment instructions ("don't
    // silently fix it"). See README.md for the flag and the fix applied in
    // reddit/web/youtube.
